...
const fs = require("fs");
const { compareStickerCodesByAlbumOrder } = require("./core");
...
async function initDatabase() {
  ...
  await ensureColumnExists("user_stickers", "quantity", "TINYINT UNSIGNED NOT NULL DEFAULT 1");
}
...
function mapUserRow(row, stickers = []) {
  const sortedStickers = [...stickers].sort((left, right) => {
    if (left.sticker_type !== right.sticker_type) {
      return left.sticker_type.localeCompare(right.sticker_type, "pt-BR");
    }

    return compareStickerCodesByAlbumOrder(left.sticker_code, right.sticker_code);
  });
  const duplicateStickerRows = sortedStickers.filter((item) => item.sticker_type === "duplicate");
  return {
    id: row.id,
    name: row.name,
    email: row.email,
    ...
    missingStickers: sortedStickers
      .filter((item) => item.sticker_type === "missing")
      .map((item) => item.sticker_code),
    duplicateStickers: duplicateStickerRows.map((item) => item.sticker_code),
    duplicateStickerQuantities: duplicateStickerRows.reduce((result, item) => {
      result[item.sticker_code] = Number(item.quantity || 1);
      return result;
    }, {})
  };
}
...
async function replaceUserCollection(
  ...
  await withTransaction(async (connection) => {
    ...
    const values = [
      ...missingStickers.map((code) => [userId, code, "missing", 1]),
      ...Object.entries(duplicateStickerQuantities || {}).map(([code, quantity]) => [
        userId,
        code,
        "duplicate",
        quantity
    ...
    if (values.length > 0) {
      await connection.query(
        `
          INSERT INTO user_stickers (user_id, sticker_code, sticker_type, quantity)
          VALUES ?
        `,
        [values]
      );
...
async function updateUser(user) {
  ...
  const duplicatesChanged =
    JSON.stringify(currentUser?.duplicateStickerQuantities || {}) !==
    JSON.stringify(user.duplicateStickerQuantities || {});

  if (missingChanged || duplicatesChanged) {
    await replaceUserCollection(user.id, user.missingStickers, user.duplicateStickerQuantities);
  }
